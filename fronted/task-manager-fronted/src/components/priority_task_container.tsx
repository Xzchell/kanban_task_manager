...
const PriorityTaskContainer : React.FC<IPriorityTask> = ({priority, onPriorityChange}) => {
                    ...
                    <div style={styles.dropdown}>
                        {Object.entries(config).map(([id, info]) => (
                                <div 
                                    key={id} 
                                    onClick={() => {
                                        onPriorityChange?.(Number(id));
                                        setIsOpen(false);
                                    }} 
                                    style={{
                                        ...styles.option,
                                        backgroundColor: Number(id) === priority ? info.backgroundColor : 'transparent',
                                        color: Number(id) === priority ? info.color : '#1e293b'
                                    }}
                                >
                                <div style={styles.contentLeft}>
...
export default PriorityTaskContainer;
const styles = {
    wrapper: {
        display: 'flex',
        alignItems: 'center',
